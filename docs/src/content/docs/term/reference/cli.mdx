...
| Argument | Description |
|---|---|
| `<term>` | The entry's `id`, or its preferred `label` in any case. An alt-label does not find the entry. |
...
`sarif` and `junit` are the metadata tool's
[renderers](/manni/meta/reference/output-and-exit-codes/) over the same
findings, under the rule id `manni:term/<rule>`. JUnit testcases carry the
classname `manni.term`, one per file read.
...
| `--dry-run` | n/a | off | Write nothing. Print what would change, and exit `0`. |
...
A render names the target and the number of entries. A render into one file
per entry says so. Dropped fields follow on an indented line:
...
`-f vale` lists each style file and what it holds. When no section of Vale's
...
differs they print that the target is up to date:
...
| `term get missing` | `manni: no term "missing". 2 terms; did you mean "progressive-lens"?` |
