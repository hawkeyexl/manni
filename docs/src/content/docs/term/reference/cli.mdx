...
finding sits on an entry. `summary` holds `terms`, `references`, `errors`,
...
| `-c, --config` | `<path>` | discovered | Path to a config file. |
| `--no-color` | n/a | off | Disable colored output. |
| `-f, --format` | `<markdown\|mdx\|asciidoc\|rst\|html\|dita\|docbook\|tbx\|skos\|csv\|json\|vale>` | each entry's own, in place | What to render. |
...
`-f vale` lists each style file and what it holds. `Casing.yml` and
`Lowercase.yml` count the labels and alt-labels they swap. When no section of Vale's
configuration uses the `Terms` style, a notice on stderr names the line to
add:
