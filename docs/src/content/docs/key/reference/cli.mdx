...
citation `src`, with its keyed pin. Pages are re-encrypted in memory first, and
nothing is written unless every value could be. See
...
Pages are re-encrypted in memory first. A value with the ciphertext shape that
decrypts under neither the current key nor the new one is skipped. A single
skip means nothing is written, not a page and not the key. The run exits `1`,
and the pages stay under the current key until the value is fixed. The new key
is never written beside a value it cannot decrypt.
...
itself reaches the config before any page is encrypted under it. The old key
stays beside it until no page needs it.
...
key, and its last line says how to finish:
...
`--dry-run` re-encrypts every page in memory and reports what would change.
It writes nothing at all, not a page and not the config, and its last line is
`Dry run: nothing written.` A value it would skip still exits `1`.
