...
citation `src`, with its keyed pin, and every encrypted value in the selected
collections' local
[external-metadata manifests](/manni/meta/reference/configuration/#external-metadata).
Everything is re-encrypted in memory first, and nothing is written unless every
...
new key changes every pin. When a baseline exists, a run that wrote pages ends
with this line:
...
and keeps its line range after the ciphertext. A citation with no id is named
by where it sits: `/citations/0` in the frontmatter list, or `line 12` inline.
In JSON a citation row carries `kind: "citation"`, its `id`, `index` or `line`,
and `from` and `to`; a skipped row carries `kind` and `message` in place of
`from` and `to`.
