...
| `structure.tool` | string, `manni` | `manni` | no | Which tool performs the job. `manni` is this package's own engine and the only value. Anything else is refused by name. `--tool <name>` names the same thing for one run. |
