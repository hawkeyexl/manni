...
description: "Every lint: key of manni.config.yaml, the collections: list the document set comes from, the template resolution chain, and the keys that moved."
---
...
built-in template serves needs no `lint:` section at all, and an empty one is
the same as none.
...
| `templates` | string[] | none | no | Template files to load. Their templates' `types:` join the doctype routing table and outrank the built-ins, so adopting a repository's own how-to shape is one entry here and no flag. Each entry is a [template ref](/manni/lint/reference/templates/#template-refs). `--templates <path>` adds more. |
...
| `overrides[].files` | string | — | yes | Glob matched against each file's path, written with forward slashes whatever the platform uses. |
| `overrides[].template` | string | — | yes | Template ref applied to the files the entry matches. |
...
`collections:` already owns that. Deferred deliberately in proposal 0049.
</Aside>
