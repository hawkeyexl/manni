...
`collections:` already owns that. Proposal 0050 records the decision.
</Aside>

## `tools:` keys

An outside tool's settings sit at the top level, beside `collections:`, not
...
Each tool gets a namespace of its own keys rather than a shared shape, because
tools do not have the same settings.
...
| `"error" \| "warning"` severities of its own | The [family scale](/manni/meta/reference/output-and-exit-codes/#severity-across-the-family). Every structural finding is `error` but one, and that warning never changes the exit code. |
| A template as a map of section rules | A [grammar](/manni/lint/reference/templates/). `sections:` is an ordered list, `required:` and `additionalSections` are gone, and `repeat:` groups rules rather than taking a boolean. The loader refuses each old key by name and prints the new spelling. |
