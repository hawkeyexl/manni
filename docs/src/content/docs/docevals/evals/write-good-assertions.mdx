...
sentence is usually obvious. See [Fix a failing eval](/manni/docevals/fix/).
...
sure. See [Deterministic checks](/manni/docevals/evals/deterministic-checks/), and
[promote](/manni/docevals/adopt/promote-to-deterministic/) for evals already on the wrong tier.
...
reverse. See [Retrofit a legacy corpus](/manni/docevals/adopt/retrofit-a-legacy-corpus/).
...
[`calibrate`](/manni/docevals/judge/calibrate/) refuses to certify a golden set with no failing
...
`type` defaults to `regression`. See [Regression vs capability](/manni/docevals/evals/regression-vs-capability/).
...
are unsure whether a rewrite helped, [calibrate](/manni/docevals/judge/calibrate/) will tell you.
