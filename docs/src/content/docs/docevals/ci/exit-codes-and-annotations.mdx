...
```yaml
      ...
      - id: Fail the PR on findings
        if: steps.manni/docevals.outcome == 'failure'
        run: |
          code=${{ steps.manni/docevals.outputs.exit-code || '1' }}
          if [ "$code" = "2" ]; then
...
If the repo has a [findings baseline](/manni/docevals/reference/files-and-state/#findings-baseline),
...
```text
::error file=docs/actions/goTo.mdx,line=4,title=manni docevals%3A fresh-enough::Page last reviewed 937 days ago (max 365)
```
...
properties, which is why the title renders as `manni docevals%3A fresh-enough`.
...
[Write good assertions](/manni/docevals/evals/write-good-assertions/).
...
{/* step {"description":"Exit 0: every eval passes.","runShell":{"command":"manni docevals run test/docevals/fixtures/pages/docs/actions/find.mdx --deterministic-only --no-generate","exitCodes":[0]}} */}

{/* step {"description":"Exit 1: a finding, or a suite below its target.","runShell":{"command":"manni docevals run test/docevals/fixtures/pages/docs/actions/goTo.mdx --deterministic-only --no-generate","exitCodes":[1],"stdio":"below target"}} */}

{/* step {"description":"Exit 2: an operational error, distinct from a finding.","runShell":{"command":"manni docevals run -c does-not-exist.yaml","exitCodes":[2]}} */}

{/* step {"description":"github format emits an ::error with file and line for a finding with a known line.","runShell":{"command":"manni docevals run test/docevals/fixtures/pages/docs/actions/goTo.mdx --deterministic-only --no-generate --format github","exitCodes":[1],"stdio":"/::error file=.*,line=4,/"}} */}

{/* step {"description":"An eval renamed out from under a pipeline exits 2, so the job stops rather than passing over nothing.","runShell":{"command":"manni docevals run test/docevals/fixtures/pages --deterministic-only --eval renamed-away","exitCodes":[2],"stdio":"matched no evals"}} */}
