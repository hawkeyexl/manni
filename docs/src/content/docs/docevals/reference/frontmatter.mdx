...
| `provider` | string | `docevals.provider` | Which provider judges this `ai` eval: `auto`, `anthropic`, `openai`, `claude-cli`, or `llama-cpp`. A CLI `--provider` still wins. An unknown name makes this eval an `error`. |
