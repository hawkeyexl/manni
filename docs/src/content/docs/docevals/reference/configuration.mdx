...
file left over from before the rename, `docevals.config.yaml`, is not read. manni docevals stops
...
| `provider.llama-cpp.modelsDir` | string \| null | `null` |
| `provider.llama-cpp.thoughtTokens` | integer ≥ 0 | `0` |
