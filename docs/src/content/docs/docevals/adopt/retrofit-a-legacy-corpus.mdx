...
```console
warn .manni-docevals-baseline.json Recorded .manni-docevals-baseline.json, but no `baseline:` is set
in /repo/manni.config.yaml. Until it is, an ordinary run ignores this file — add the key, or pass
--baseline.
```
...
are in [Files and state](/manni/docevals/reference/files-and-state/#findings-baseline).
...
is not what you want anyway. [Human review](/manni/docevals/judge/human-review/) is the mechanism
for those.
...
Exclude them from the [collection](/manni/docevals/reference/configuration/#which-pages-a-run-reads)
...
[Bootstrap a corpus](/manni/docevals/adopt/).
...
eval](/manni/docevals/fix/) is the page to hand contributors.
