...
- **`collection: <name>`** points at a set declared in the top-level [`collections:`](/manni/meta/reference/configuration/#collections) list. Reach for this when the area is a real document set the whole repo recognises, such as the API reference, the blog, or the author pages. The globs live in one place, every manni tool sees the same set, and `manni meta query` exposes it as a view of the same name.
- **`files: <glob>`** takes a glob, or a list of them. Reach for this when the subset exists only to change schema choice: one directory that needs an extra check, or a naming convention that cuts across collections.
...
Here the API reference and the guides are areas the whole repo thinks in, so they are collections, and the overrides point at them by name:
