...
```sh
npx manni meta schemas infer docs/
```
...
```sh
npx manni meta schemas infer docs/ --out ./schemas/permissive.json
```
...
```bash
npx -y manni meta validate --format github
```
...
2. **Exclude the corners you are not ready for.** Your `exclude` globs merge with manni meta's built-in ignores (`**/node_modules/**` and `**/.git/**` are always skipped) and add to them.
...
```bash
npx manni meta validate --write-baseline
```
...
{/* step {"description":"The coverage probe reports what the docset has, and counts the files with no metadata block at all.","runShell":{"command":"npx manni meta schemas infer test/fixtures/infer --no-config","exitCodes":[0],"stdio":"with no metadata block"}} */}
{/* step {"description":"A frontmatter-free file passes the permissive schema (nothing is required).","runShell":{"command":"npx manni meta validate test/fixtures/no-frontmatter.md -s ./test/fixtures/dd/permissive.schema.json","exitCodes":[0],"stdio":"1 passed"}} */}
{/* step {"description":"The same file fails OKF, which requires `type` — the avalanche the permissive start avoids.","runShell":{"command":"npx manni meta validate test/fixtures/no-frontmatter.md -s google:okf:0.1","exitCodes":[1],"stdio":"must have required property 'type'"}} */}
{/* test end */}
