...
concepts: [frontmatter, extractor]
...
intent: Understand which file formats manni meta supports and how metadata is extracted from each format
visibility: public
lifecycle: published
applies-to: manni-meta
prerequisites: meta-get-started
related-pages:
  - /manni/meta/reference/cli/
  - /manni/meta/reference/element-metadata/
  - /manni/meta/reference/output-and-exit-codes/
  - /manni/meta/reference/dita-schema/
risks: read-only
sample-questions:
  - What file formats does manni meta support?
  - How does manni meta read metadata from XML files?
  - What is the difference between YAML, TOML, and JSON frontmatter?
  - How does type coercion work in native metadata paths?
  - Can manni meta write metadata back to DITA files?
meta-provenance:
...
{/* cite formats-extractor-chosen-by-extension-or-as */}
...
[`--as <format>`](/manni/meta/reference/cli/#shared-input-model). Every extractor below
counts toward directory and glob walks.
...
{/* cite formats-writability-is-the-apply-method */}
...
{/* cite formats-fenced-frontmatter-round-trips */}
...
{/* cite formats-native-syntaxes-are-not-round-trippable */}
...
{/* cite formats-xml-ranges-rebuilt-from-start-positions */}
...
{/* cite formats-dita-metadata-belongs-in-prolog */}
...
{/* cite formats-html-and-xml-are-spliced-not-reserialized */}
...
{/* cite formats-schemas-reports-writability */}
...
{/* cite formats-fill-writes-where-the-read-found-it */}
...
{/* cite formats-as-name-and-case-insensitive-extensions */}
The `--as` name is the extractor name in the first column. The extension match
is case-insensitive.

## Frontmatter flavors

{/* cite formats-three-interchangeable-flavors */}
...
{/* cite formats-yaml-also-closes-on-dot-dot-dot */}
...
{/* cite formats-an-unterminated-fence-is-not-frontmatter */}
...
{/* cite formats-toml-line-map-is-best-effort */}
...
{/* cite formats-mdx-reuses-markdown-frontmatter */}
...
{/* cite formats-asciidoc-accepts-two-styles */}
...
{/* cite formats-rst-accepts-two-styles */}
...
{/* cite formats-xmlns-declarations-are-dropped */}
...
{/* cite formats-malformed-xml-is-a-per-file-error */}
...
{/* cite formats-dita-read-by-the-xml-extractor */}
...
{/* cite formats-unresolvable-entities-do-not-fail-the-file */}
...
{/* cite formats-elements-are-read-too */}
...
{/* cite formats-othermeta-wins-over-a-root-attribute */}
...
{/* cite formats-typed-prolog-elements-are-separate-keys */}
...
{/* cite formats-dita-fill-three-cases */}
...
{/* cite formats-dita-write-lands-where-the-read-was */}
...
{/* cite formats-three-ways-a-file-counts-as-dita */}
...
{/* cite formats-html-parsing-never-throws */}
HTML parsing recovers from malformed markup, so extraction does not throw a
parse error.

{/* cite formats-new-meta-goes-just-inside-head */}
...
{/* cite formats-native-paths-parse-as-yaml-scalars */}
...
{/* cite formats-quoting-decides-the-type */}
...
{/* cite formats-toml-native-dates-are-rewritten-before-validating */}
TOML's native date-time type is the one value a fenced block still rewrites. An
unquoted `published = 2024-01-15T10:00:00Z` parses as a date, and manni meta
writes it back to its ISO spelling before validating. A field typed
`{"type": "string", "format": "date-time"}` therefore accepts it, exactly as it
accepts the quoted form.

{/* cite formats-iso-spelling-always-carries-milliseconds */}
The two spellings are not byte-identical, though. The ISO spelling always
carries milliseconds, so the unquoted value above is validated as
`2024-01-15T10:00:00.000Z`. `format: date-time` does not notice. `enum`,
`const` and `pattern` do, so quote the value when the exact text is what your
schema matches.
