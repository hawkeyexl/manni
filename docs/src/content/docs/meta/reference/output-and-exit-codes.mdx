...
| `manni lint check` | `error` only, today | Every structural finding is an error, and there is no severity map. The reporters carry all three for the jobs still to come, which is why [exit `1`](/manni/lint/reference/cli/#exit-codes) is defined on severity rather than on "any finding". |
...
| `pretty`, `lint` | not emitted yet | not emitted yet | the file line is `✗`; each finding is `line:col`, the rule id, the message |
...
| `manni lint check` | A file with at least one error-severity finding. Stated on severity before any job needs the distinction, which is cheaper than changing it afterwards. |
