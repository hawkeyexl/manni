...
is the third reserved first segment, beside `check` and `external`.
...
validation produces is an `error`. The lower levels exist for sibling tools in
...
|---|---|---|
| `manni meta validate` | `error` only | Every finding this tool's own validation produces is an error. The reporters carry the other two for sibling tools. |
...
So a repository that runs only `manni meta validate` sees one level, and adding
a second tool is what puts the other two on screen.

### How each level renders

| Format | `notice` | `warning` | `error` |
|---|---|---|---|
| `pretty`, this tool | the word `notice`, dim, before the message | the word `warning`, yellow | no word; the file line is `✗` |
...
| Config `derive.commands` names a field a built-in source already derives. | `derive.commands.last-updated targets a field git already derives; a command may only derive a field no built-in source claims`. `$schema` is refused the same way, and a manifest-owned key when the config loads. |
...
| `derive` was given a field that is not derivable. | `"stakeholders" is not derivable; derivable fields are created, last-updated, authors, owner, reviewed-by, last-reviewed, or any key with an entry in derive.commands` |
| Config `derive.fields` names a key a collection's [external-metadata manifest](/manni/meta/reference/configuration/#external-metadata) owns. The config is refused when it loads. | `manni.config.yaml: meta.derive.fields[3] "owner" is owned by collections[0].externalMetadata[0] (./owners.yaml) — a managed field has one authority, and a manifest key already has one. Drop it from one side.` |
| Config `derive.commands` names a key a collection's [external-metadata manifest](/manni/meta/reference/configuration/#external-metadata) owns. The config is refused when it loads. | `manni.config.yaml: meta.derive.commands.verified-against is owned by collections[0].externalMetadata[0] (./versions.yaml) — a managed field has one authority, and a manifest key already has one. Drop it from one side.` |
| `derive --fields` named a key a collection's [external-metadata manifest](/manni/meta/reference/configuration/#external-metadata) owns. | `"owner" is owned by the manifest ./owners.yaml on collection guides; a managed field has one authority, and a manifest key already has one.` |
