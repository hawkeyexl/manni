...
| `manni lint check` | all three | Every finding from manni's own structure check is an error except `unsupported-content-kind`, the warning for a rule the file's format cannot answer. Under DITA-OT, a message keeps its log level: `ERROR` and `FATAL` are errors, `WARN` is a warning and `INFO` is a notice. There is no severity map. [Exit `1`](/manni/lint/reference/cli/#exit-codes) is defined on severity rather than on "any finding", so a warning or a notice never fails a run. |
...
| `pretty`, `lint` | the word `notice`, dim, before the message | the word `warning`, yellow; a file with warnings and no errors is marked `⚠` | no word; the file line is `✗` |
| `json` | `"severity": "notice"` | `"severity": "warning"` | the field is absent in `meta`, and `"error"` in `cite`, `a11y` and `lint` |
...
by this tool, by `manni cite` and by `manni lint`; `manni a11y check` reports in
`pretty`, `json` and `github`.
...
| `manni lint check` | A file with at least one error-severity finding. |
