...
| `manni lint check` | `error` and `warning` | Every structural finding is an error except `unsupported-content-kind`, the warning for a rule the file's format cannot answer. There is no severity map. [Exit `1`](/manni/lint/reference/cli/#exit-codes) is defined on severity rather than on "any finding", so that warning never fails a run. |
...
| `pretty`, `lint` | not emitted | not emitted | the file line is `✗`; each finding is `line:col`, the rule id, the message |
