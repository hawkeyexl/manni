...
{/* cite output-pretty-marks-pass-and-fail */}
...
{/* cite output-a-manifest-value-is-located-in-the-manifest */}
...
{/* cite output-summary-line-shape */}
...
{/* cite output-gitignore-skips-named-on-the-summary */}
...
{/* cite output-gitignore-clause-omitted-at-zero */}
...
{/* cite output-json-is-one-object */}
...
| `summary.errors` | number | Error-severity findings reported across all files. Findings a [baseline](/manni/meta/reference/cli/#baselines) forgave are **not** counted here, and neither are warnings or notices. |
| `summary.warnings` | number | Warning-severity findings reported across all files. A warning never fails a file or the run. **Omitted** when there are none, which is every run whose schemas carry no `x-manni-location` marks. |
| `summary.notices` | number | Notice-severity findings reported across all files. A notice never fails a file or the run. **Omitted** when there are none, which is every run of `manni meta validate` today. |
...
{/* cite output-keyword-and-subject-are-machine-stable */}
...
{/* cite output-github-annotation-levels */}
...
{/* cite output-github-message-escaping */}
...
{/* cite output-sarif-findings-are-tracked-alerts */}
...
| Field | Type | Description |
|---|---|---|
| `runs[].tool.driver.name` | string | Always `manni`. |
...
| `results[].level` | string | `error`; `warning` for a finding whose `severity` is `warning`; `note` for one whose `severity` is `notice`. See [severity](#severity) below. |
...
{/* cite output-encrypted-rule-ids */}
...
{/* cite output-derived-stale-rule-id */}
...
adds the first file line of the range, and the subject `provenance
<integrity>`, so each range has its own fingerprint.

{/* cite output-stdin-produces-no-sarif-result */}
...
{/* cite output-sarif-paths-are-repo-relative */}
...
{/* cite output-unresolvable-sarif-paths-are-reported */}
...
{/* cite output-fingerprint-excludes-the-path */}
...
{/* cite output-sarif-level-follows-severity */}
...
{/* cite output-junit-renders-in-the-tests-tab */}
...
|---|---|---|
| `name` | `testsuites`, `testsuite` | Always `manni`. |
| `tests` | `testsuites`, `testsuite` | Files checked, as **one test per file** rather than per violation. |
| `failures` | `testsuites`, `testsuite` | Files with at least one error-severity violation. A warning or a notice is not a `<failure>`, because JUnit has no level below failure. A file holding only warnings or notices is a passing testcase, matching the exit code. |
...
{/* cite output-one-junit-test-per-file */}
...
{/* cite output-baseline-block-only-with-a-baseline */}
...
{/* cite output-one-severity-scale-for-the-family */}
Every manni tool speaks one severity scale, `notice | warning | error`, least
...
| `manni cite check` | all three | Per rule. Each rule has a default, and [`cite.severity.<rule>`](/manni/cite/reference/configuration/#severity) moves any of them, or sets it to `off`. |
| `manni a11y check` | all three | Mapped from axe's `impact` when a page is read. Its [`--severity`](/manni/a11y/reference/cli/) flag then drops everything below a floor. |
| `manni key rotate` | none | It has no findings model. A value either re-encrypts or is skipped, and a skip exits `1`. |

{/* cite output-meta-alone-sees-errors-and-warnings */}
...
{/* cite output-sarif-has-no-notice-level */}
...
{/* cite output-summary-counts-lower-levels-only-when-present */}
A pretty summary line counts the lower levels only when there are some. This
tool prints `2 files checked, 2 passed, 0 failed, 0 errors, 1 warning, 2
notices`, and `manni cite check` prints the counts in parentheses, as
...
| `manni a11y check` | A page that failed to load, or one with any remaining violation. The floor is the gate, so `--severity` is what stops a lower level failing the run. |

{/* cite output-no-fail-on-flag */}
...
`manni a11y check` is the one tool whose floor is also its gate, and that is
deliberate. Here and in `manni cite check` you can move a finding
onto the level you want. A rule you care about becomes an error, through
`cite.severity.<rule>` or by writing it as a requirement in the schema. The
exit code can sit at `error` and stay useful. axe assigns a11y's severities
from its own `impact`, and no config moves one. Pinning that exit code to
`error` would leave the domain with no way to gate on an axe `moderate`
finding. So `--severity` does both jobs there, and the floor a run reports at
is the floor it fails at. pa11y and Vale, the two linters this scale was
borrowed from, read their own level the same way.

The practical shape is a ratchet. Set `severity: error` to gate on axe's
`critical` and `serious` alone, fix what it finds, then lower the floor to
`warning` and to `notice` as each level reaches zero. Lower it as findings are
fixed; never raise it to get a build green.
[Proposal 0064](https://github.com/hawkeyexl/manni/blob/main/docs/proposals/0064-severity-and-the-exit-code.md)
is the record.

### How a domain maps its own scale on

{/* cite output-a-foreign-scale-folds-on */}
A tool whose source speaks another scale folds onto these three and keeps the
source's own value in a field of its own. Nothing is lost for lookup, and one
word still means one thing across the family.
...
{/* cite output-a11y-keeps-impact-beside-severity */}
...
{/* cite output-cite-assigns-severity-per-rule */}
...
configuration reference](/manni/cite/reference/configuration/#severity) has all
...
{/* cite output-parser-rejections-are-also-exit-2 */}
...
{/* cite output-exit-1-vs-exit-2-meaning */}
...
{/* cite output-a-baseline-relaxes-no-schema */}
...
{/* cite output-fingerprint-includes-the-schema-ref */}
...
{/* cite output-zero-files-is-exit-2 */}
...
{/* cite output-nothing-checked-is-exit-2-not-1 */}
...
{/* cite output-a-url-schema-is-checked-before-compiling */}
...
{/* cite output-a-non-schema-would-exit-0 */}
...
| A `query` `_path` move would leave a page's values in a manifest per page, which is named after the old path. | `docs/a.mdx cannot move to docs/b.mdx, because its "citations" lives in docs/a.citations.yaml, a {page} manifest named after the page's path. Move the page and its manifest together, then rename the entry's key.` |
...
{/* cite output-collection-refusals-are-usage-errors */}
...
{/* cite output-a-source-that-cannot-answer-is-exit-2 */}
With [`derive:`](/manni/meta/reference/configuration/#derive) in config,
`validate`, `derive`, `get`, and a `query` naming the
[`derived`](/manni/meta/reference/query/#the-derived-table) or
[`resolved`](/manni/meta/reference/query/#the-resolved-table) table read
evidence from git, a CODEOWNERS file, the `gh` or `glab` CLI, and
any [command](/manni/meta/reference/configuration/#commands) the config
...
| Config `derive.fields` names a key a collection's URL [external-metadata manifest](/manni/meta/reference/configuration/#external-metadata) owns. The config is refused when it loads. A local manifest is where `derive` writes the field. | `manni.config.yaml: meta.derive.fields[3] "owner" is owned by manifest https://example.com/owners.yaml, which is fetched and cannot be written; set it in that repository.` |
| Config `derive.commands` names a key a collection's URL [external-metadata manifest](/manni/meta/reference/configuration/#external-metadata) owns. The config is refused when it loads. | `manni.config.yaml: meta.derive.commands.verified-against is owned by manifest https://example.com/versions.yaml, which is fetched and cannot be written; set it in that repository.` |
| `derive --fields` named a key a collection's URL [external-metadata manifest](/manni/meta/reference/configuration/#external-metadata) owns. | `"owner" is owned by manifest https://example.com/owners.yaml, which is fetched and cannot be written; set it in that repository.` |
...
{/* cite output-stale-stamp-is-exit-1 */}
...
{/* cite output-color-only-for-pretty */}
...
{/* cite output-machine-formats-are-never-colored */}
...
{/* cite output-machine-formats-own-stdout */}
