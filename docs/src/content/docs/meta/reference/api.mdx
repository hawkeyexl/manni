...
| Export | Purpose |
|---|---|
| `cite` | The citation tool's programmatic API. It carries the command cores `runCheck`, `runAdd` and `runUpdate`; the per-page check `checkCitations` and the minter `mintCitation`; the primitives `hashRange`, `parseSrc`, `obfuscatePath` and `parseStatements`; `toValidationResult`, which turns a page report into the `ValidationResult` above so the reporters and the baseline work unchanged; and `CiteError`, its exit-2 failure. `import { cite } from "@hawkeyexl/manni"`. See [the cite reference](/manni/cite/reference/cli/). |
