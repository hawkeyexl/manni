...
| `DERIVE_SOURCES` | The sources `derive.sources` may name: `git`, `codeowners`, `github`, `gitlab`. Absent in config means all four, of which only the `github` or `gitlab` the origin remote matches is consulted. |
...
| `DeriveContext` (type) | What a derive run holds constant: `cwd`, `base`, `configDir`, the `sources` and `fields` in play, an optional `codeowners` path, whether to `cache`, a `now` clock, and an optional `reviews` client. |
...
| `consultedSources` | The sources a run consults, meaning those requested that some requested field can come from, in the requested order. |
...
| `LoadedConfig` (type) | A parsed config plus where it came from. It carries the file's `path`, its `dir`, and the `source` label every config message spells it as. It also carries the family-file `section` it was read from under, and every `collections` the family file declares. |
