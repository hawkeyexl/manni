...
| `FieldError` (type) | One violation. The JSON Pointer, the message, the failing keyword, the schema that produced it, and, when a producer sets one, its `severity`. |
| `isErrorSeverity` | Whether a `FieldError` counts against `ok`: true for `error` and for an absent severity, false for `warning`. The one reading of the severity invariant; derive `ok` through it. |
