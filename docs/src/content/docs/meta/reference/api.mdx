...
| `KG_OUTPUT_KEYWORD` | The schema keyword `x-manni-kg-output`, a boolean beside a top-level property saying whether a published knowledge graph may carry that field. Absent means `true`. `Validator.kgOutputPreferences` takes the data and the schema refs and resolves to a map from each marked top-level key to its boolean. An unmarked key has no entry, a later ref wins, and a schema whose evaluated branches say both gives that key no preference. Annotation only here. `manni kg` is what acts on it. |
...
| `docevals` | The evals tool's programmatic API. It carries `runRun`, `runList` and `loadConfig`, the judge and provider factories, the reporters, and the page vocabulary it validates against (`frontmatterSchema`, `FRONTMATTER_SCHEMA_ID`). `import { docevals } from "@hawkeyexl/manni"`. See [the docevals reference](/manni/docevals/reference/). |
| `kg` | The knowledge-graph tool's programmatic API. It carries the command cores `runBuild`, `runQuery`, `runStats`, `runSearch`, `runTraverse`, `runEmbed`, `runExport` and `runFill`. It carries the pipeline too: `analyzeDoc`, `deriveGraph`, `collectGitHistory`, and the emitters `emitTurtle`, `emitJsonLd` and `emitRdfXml`. Then the IRI minters, the browser runtime, the embedders, and the page vocabulary it validates against (`frontmatterSchema`, `FRONTMATTER_SCHEMA_ID`). `KgError` is its exit-2 failure. `import { kg } from "@hawkeyexl/manni"`. The browser half is also published on its own, dependency-free, as `@hawkeyexl/manni/kg/runtime` and `@hawkeyexl/manni/kg/embed`. See [the kg reference](/manni/kg/reference/library-api/). |
