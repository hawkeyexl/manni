...
| `docevals` | The evals tool's programmatic API. It carries `runRun`, `runList` and `loadConfig`, the judge and provider factories, the reporters, and the published frontmatter schema (`frontmatterSchema`, `frontmatterSchemaPath`). `import { docevals } from "@hawkeyexl/manni"`. See [the docevals reference](/manni/docevals/reference/). |
