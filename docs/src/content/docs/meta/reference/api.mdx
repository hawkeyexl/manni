...
| `deriveMetadata` | The orchestrator. It resolves every requested field for every input from the sources the run consults. A source is consulted only when a requested field can come from it, and the forge rides on git. Precedence is per field, with the first non-null answer winning. |
