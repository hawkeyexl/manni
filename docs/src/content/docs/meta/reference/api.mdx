...
| Export | Purpose |
|---|---|
| `cite` | The citation tool's programmatic API. It carries the command cores `runCheck`, `runAdd`, `runUpdate`, `runSaltSet` and `runSaltRotate`; the per-page check `checkCitations` and the minter `mintCitation`; the config writer `writeConfigSalt`; and the primitives `hashRange`, `parseSrc`, `obfuscatePath`, `generateSalt` and `parseStatements`. `toValidationResult` turns a page report into the `ValidationResult` above, so the reporters and the baseline work unchanged. `CiteError` is its exit-2 failure. `import { cite } from "@hawkeyexl/manni"`. See [the cite reference](/manni/cite/reference/cli/). |
