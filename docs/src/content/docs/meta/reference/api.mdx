...
| `tracevals` | The session-eval tool's programmatic API. It carries the command cores `runRun`, `runBatch`, `runCalibrate`, `runFill`, `runCapture` and `runList`; the engine `runEvals`; the trace reader `parseTraceFile` and the store scan `discoverTraces`; artifact resolution, the grader registry (`registerGrader`, `loadGraderPlugins`) and the reporters; and the artifact vocabulary it validates against (`ARTIFACT_EVALS_SCHEMA_ID`). `TracevalsError` is its exit-2 failure. A grader plugin registers through this namespace, so it binds to the registry the CLI reads. `import { tracevals } from "@hawkeyexl/manni"`. See [the tracevals reference](/manni/tracevals/reference/api/). |
