...
`locate`, and runs the orphan check when the run is the config corpus.
...
| `SidecarEntry` (type) | One manifest entry. It carries the absolute document path it names and the key as spelled in the manifest. It also carries the manifest's reported path and the entry's line when known. |
...
| `SidecarConfig` (type) | One [sidecar](/manni/meta/reference/configuration/#sidecars) entry, made of the manifest `file`, the top-level `keys` it owns, and an optional `tokenEnv`. `file` is a path relative to the config's directory or an `https://` URL, and `tokenEnv` names the environment variable holding the bearer token for a URL. The parser has already asserted the keys are non-empty, unique, disjoint across entries, and never `$schema`, and that `tokenEnv` appears only on a URL. |
