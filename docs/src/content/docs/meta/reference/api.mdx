...
| `runDerive` | Stamp the managed stewardship fields from git history, CODEOWNERS and the forge. Backs `manni meta derive`. Writes a `stale` or `unset` field unless `dryRun` or `check`; leaves `current` and `unknown` alone. Throws `DocmetaError` for stdin, a field or source it does not know, nothing to derive, and a consulted source that cannot answer. |
| `DeriveOptions` (type) | Input to `runDerive`: the same input model minus stdin, plus `fields` and `sources` (config `derive:` otherwise), `dryRun`, `check`, `cache`, a `now` clock, and a `forge` client to inject. |
...
| `DeriveSummary` (type) | Aggregate counts: files, changed, fields written, and how many fields were stale, unset, or unknown, plus errors. |
...
| Export | Purpose |
|---|---|
| `Validator` | The Ajv-backed engine. Validate already-extracted metadata against a schema set without going through the file pipeline. `validate` takes the data, the schema refs, a `lineFor` callback, an optional `colFor`, and an optional fifth `locate` callback. `locate` says where a merged value lives when it is not in the document (a [sidecar](#sidecars) manifest's file and line). |
...
| `FieldError` (type) | One violation. The JSON Pointer, the message, the failing keyword, and the schema that produced it. An optional `file` names the file `line` and `col` refer to when it is not the result's own. It is set only for a value a sidecar manifest supplied. |
...
The manifest that keeps frontmatter values outside the document, joined to
documents at run time. See
...
once per run and merges per document. It validates the merged object with
`locate`, and runs the orphan checks when the run is the config corpus.
...
| `LoadSidecarsOptions` (type) | Input to `loadSidecars`. `configDir` is the directory `file` and every manifest key resolve from. `base` is the directory the run's file labels are relative to, so manifests are reported the same way. `offline` refuses a URL `file` rather than fetching it. `timeoutMs` bounds the fetch, and `env` is the environment `tokenEnv` is read from, for tests. |
| `fetchSidecar` | Fetch one [remote manifest](/manni/meta/reference/configuration/#remote-manifests) by URL, sending the bearer token `tokenEnv` names to that origin only. Resolves to the manifest text. Throws `DocmetaError` naming the URL and the status, never the token, and under `offline` before any request. `loadSidecars` calls it for a URL `file`. |
| `SidecarFetchOptions` (type) | Input to `fetchSidecar`, made of `tokenEnv`, `timeoutMs`, `maxBytes`, `offline`, and `env`, the environment `tokenEnv` is read from, which defaults to `process.env`. |
| `sidecarUrlProblem` | Why a string may not name a remote manifest, or `null` when it may. It refuses a URL carrying userinfo, plain `http://` off a loopback host, and any other scheme. The config parser applies it to every URL `file`. |
| `SIDECAR_FETCH_TIMEOUT_MS`, `SIDECAR_FETCH_MAX_BYTES` | The fetch's defaults, 10 000 ms and 5 MB, matching the schema fetch. |
| `SidecarIndex` (type) | Every sidecar of a run. `owners` maps each owned key to its manifest. `byPath` maps an absolute document path to its supplied values. `byField` maps a [join field](/manni/meta/reference/configuration/#joining-by-a-field) to its values and each value to the supplied values. `entries` lists every manifest entry for the orphan checks. |
| `SidecarEntry` (type) | One manifest entry. `join` is `path` or the field it is keyed by, and `abs` is the absolute document path of a path entry, absent for a field entry. It also carries the key as spelled, the manifest's reported path, and the entry's line when known. |
| `SidecarValue` (type) | One supplied value, with the manifest's reported path and the key's line when known. |
| `sidecarJoin` | The join a `SidecarConfig` asks for: its `join`, or `PATH_JOIN` when absent. |
| `PATH_JOIN` | The join that names documents by path, `path`, and the default. |
| `mergeSidecars` | Merge the index's values for one document into its `ExtractedMetadata`. Returns the merged object, the `collisions` (owned keys the document itself carried, which the caller files as findings), the `joins`, and a `locate` callback. Stdin never merges by path. |
| `MergedMetadata` (type) | What `mergeSidecars` returns: `extracted`, `collisions`, `joins`, and `locate`. |
| `SidecarCollision` (type) | One key a document carries that a sidecar owns, with the owning manifest. `MergedMetadata.collisions` is a list of these. |
| `SidecarJoin` (type) | One field-joined entry a document matched: the `field`, the `value`, and the manifest `file`. `MergedMetadata.joins` is a list of these, one per sidecar at most. |
| `SourceLocation` (type) | What `locate` answers with: a `file`, and optionally `line` and `col`. |
| `sidecarPointer` | The `/<key>` instance path for a collision or duplicate finding, RFC 6901 escaped. |
...
| `SIDECAR_DUPLICATE_SCHEMA` | The `schema` ref a duplicate-join finding carries: `sidecar:duplicate`. |
| `SIDECAR_KEYWORD` | The `keyword` every sidecar finding carries: `sidecar`. Joined, they are the rule ids `sidecar:owned/sidecar` and `sidecar:duplicate/sidecar`. |

## Derived metadata

Values computed from git history, CODEOWNERS and the forge rather than written
...
| Export | Purpose |
|---|---|
| `DERIVABLE_FIELDS` | The fields `derive.fields` may name: `created`, `last-updated`, `authors`, `owner`, `reviewed-by`, `last-reviewed`. |
| `DerivableField` (type) | The union of those names. |
| `isDerivableField` | Type guard for a user-supplied field name. |
| `DERIVE_SOURCES` | The sources `derive.sources` may name: `git`, `codeowners`, `forge`. Absent in config means all three. |
| `DeriveSource` (type) | The union of those names. |
| `isDeriveSource` | Type guard for a user-supplied source name. |
| `DerivedValue` (type) | One fact a source produced: the `value`, its `source`, and the `evidence` a finding quotes, such as a commit, a CODEOWNERS line, or a pull request. |
| `DerivedRecord` (type) | Everything derived for one document: the `file` and a `fields` map. A `null` field means a source was consulted and had no fact; an absent one means no source answered. |
| `SourceStatus` (type) | Whether a source can answer at all, with a `reason` naming the fix when it cannot. |
| `ForgeClient` (type) | The forge half of a derive run: `detect` the hosting kind and project, `status` (binary on `PATH` and authenticated), and `mergedChangeFor` a commit. Reached through `gh` or `glab`; inject one through `DeriveContext.forge` or `DeriveOptions.forge` to keep the network out. |
| `ForgeIdentity` (type) | What `detect` answers: the `kind` (`github` or `gitlab`), the `host`, and the `project` path there. |
...
| `DeriveInput` (type) | One document as the sources see it: its `label`, `absPath`, `content`, and `extracted` metadata. |
| `DeriveContext` (type) | What a derive run holds constant: `cwd`, `base`, `configDir`, the `sources` and `fields` in play, an optional `codeowners` path, whether to `cache`, a `now` clock, and an optional `forge` client. |
| `DerivedStatus` (type) | How an asserted value stands against the derived one: `current`, `stale`, `unset`, or `unknown`. |
| `DerivedField` (type) | One compared field: the `field`, the `asserted` value when the document carries one, the `derived` value or `null`, its `source` and `evidence`, the `status`, and whether `derive` has `written` it. |
...
| `staleFindings` | The findings `validate` files for a document's managed fields: one `FieldError` per `stale` or `unset` field, on the line the asserted key sits on, with a message naming both values and the evidence. |
...
| `deriveMetadata` | The orchestrator: resolve every requested field for every input from the sources the run consults. A source is consulted only when a requested field can come from it, the forge rides on git, and precedence is per field with the first non-null answer winning. |
...
| `FIELD_SOURCES` | Which sources can state each derivable field, in precedence order: the forge ahead of git for `reviewed-by` and `last-reviewed`, one source for every other field. |
| `consultedSources` | The sources a run consults: those requested that some requested field can come from, in the requested order. |
...
| `DocmetaConfig` (type) | The config document itself: `paths`, `exclude`, `schemas`, `overrides`, `checks`, `sidecars`, `derive`, `fill`, `schemaTrust`, `schemaCache`, and the rest. |
...
| `SidecarConfig` (type) | One [sidecar](/manni/meta/reference/configuration/#sidecars) entry, made of the manifest `file`, the top-level `keys` it owns, an optional `tokenEnv`, and an optional `join`. `file` is a path relative to the config's directory or an `https://` URL, and `tokenEnv` names the environment variable holding the bearer token for a URL. `join` is `path` or the top-level field the manifest's keys are values of. The parser has already asserted the keys are non-empty, unique, disjoint across entries, and never `$schema`. It has also asserted that `tokenEnv` appears only on a URL, and that `join` is neither `$schema` nor an owned key. |
| `DeriveConfig` (type) | The [derived channel](/manni/meta/reference/configuration/#derive): the managed `fields`, the optional `sources` to consult, and an optional `codeowners` path relative to the config's directory. The parser has already asserted the fields are non-empty, unique, each one of `DERIVABLE_FIELDS`, and never a key a sidecar owns. |
