...
language: en
source-of-truth:
  - path: src/meta/core/resolve-schema.ts
    kind: source-code
  - path: src/meta/core/validator.ts
    kind: source-code
  - path: src/meta/core/schema-registry.ts
    kind: source-code
intent: Understand how manni meta decides which schema validates each file and configure schema resolution
visibility: public
lifecycle: published
concepts:
  - validation
...
prerequisites:
  ...
  - Familiarity with manni meta validate command
next-steps:
  - /manni/meta/schemas/
...
risks: read-only
sample-questions:
  - How does manni meta choose which schema validates a file?
  - What is the schema precedence order?
  - How do I override the default schema?
  - What are the three kinds of schema references?
  - How are JSON Schema dialects detected?
graph:
  type: reference
  concepts:
    - validation
...
{/* cite first-source-that-yields-a-schema-wins */}
...
5. **The built-in default set**: if nothing above matched, manni meta uses `google:okf:0.1`, `passo-uno:seven-action:1.0` and the eleven [manni vocabularies](/manni/meta/reference/manni-vocabularies/) together.
...
| 5 | Built-in default set | All files (last resort) | `google:okf:0.1`, `passo-uno:seven-action:1.0` and the eleven `manni:*:1.0.0` vocabularies. |
...
{/* cite override-names-files-or-a-collection */}
...
the [Configuration reference](/manni/meta/reference/configuration/).)
...
{/* cite strips-schema-before-the-validator */}
...
{/* cite dialect-read-from-the-schema-line */}
