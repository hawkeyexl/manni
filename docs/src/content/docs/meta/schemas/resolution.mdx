...
Each `overrides` entry pairs a `schemas` list with the files it governs, named
in **one** of two ways: a `files` glob, as above, or `collection: <name>`, which
governs the members of a declared [collection](/manni/meta/reference/configuration/#collections).
