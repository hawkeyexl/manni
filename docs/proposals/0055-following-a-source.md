...
| `sarif` | cite renders it, over the same findings, with repository-relative uris and the fingerprints its baseline shares. Rule id `manni:cite/source-moved`, level `warning`, `partialFingerprints` unchanged. The new path is in the message the `github` annotation carries. |
| `junit` | cite renders it, under the `manni.cite` classname, with each failure's message that same one. A `source-moved` stays a passing testcase, and a `source-changed` failure's text carries the span. |
