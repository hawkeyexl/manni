...
| [0040](0040-citations-and-drift.md) | Citations and drift: pin a claim to source lines, and `manni cite` checks the pin | Devin · D4 / Theo · T1 / Maya · M2 / Sara · S1 | Implemented (#17) |
...
**Safe to start in any order, no blockers:** 0031. Its dependencies are all shipped, and the arrows above record which rules it inherits, not what it waits on. 0011 held this slot until it shipped. 0023, another `Proposed` entry, is not in this bucket. It waits on public review rather than on an implementation slot. 0040 shipped alongside its own record on `tool/cite`; its vocabulary still waits on 0023's review, its tool does not.
