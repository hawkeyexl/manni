...
| [0056](0056-several-ids-per-marker.md) | Several ids per marker. A marker's payload becomes one or more ids separated by spaces, so a paragraph that supports several claims carries one comment instead of six. `add --marker` joins the marker that is already there | Maya · M5, M6 / Theo · T2 | Implemented (#112) |
...
| [0060](0060-a-manifest-per-page-as-built.md) | A manifest per page, as built. 0058 with its four review questions answered, two refusal rows dropped because main had them, and two refusals added: two pages resolving one file, and a `query` rename of a page whose values live in its per-page manifest | Maya · M6 / Devin · D5 | Proposed |
...
0023 ──┬─> 0047          (every vocabulary field marked page or external, as new drafts beside the old)
