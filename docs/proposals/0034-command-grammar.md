...
have a top-level verb, whether `manni validate` should exist as a shortcut,
whether a second domain may copy `meta`'s default subcommand, or how a flag
that takes several values spells them.
...
spells it: `manni a11y check <url>`, never `manni a11y <url>`. The reason is
the one 0005 recorded for `get`: a positional that is sometimes a verb and
sometimes an argument produces errors that blame the wrong thing.
...
three places: the reference page documents both, the tests pin both, and the
...
**The full interface.** Every exported type, function signature, CLI argument
and option (name, type, default, required or optional, what it does), config
key and output shape the change adds or alters. "Add a flag for X" leaves the
name, the default and the arity to be invented in the diff, where the reviewer
meets them for the first time.

**A ladder of examples, minimal to maximal.** For a command: the bare minimum
that does something useful; the common variants (config fallback, the CI
format, the scripting form); one invocation using every option at once; and
the usage errors with their exact stderr line and exit code. Each rung shows
the command and what it prints. An option table says what each flag is. The
ladder shows whether the flags compose into a command a person would type, and
it is where a missing default, an awkward pairing, or a break with `meta`'s
parity gets caught before it ships. 0005's probe table is the model.
...
chunks, the review and the commit text. Exploration, each independent
implementation chunk with its plan section pasted in, long verification runs
and the demo-video pipeline go to subagents that return a conclusion. A session
...
positional structure. The umbrella keeps the two flags every CLI has and adds
no others; `manni --help` listing the domains is the grammar's index, not an
exception to it.
...
default subcommand under another name, and the second instance of an exception
is a pattern.

### 4. Why not let `<list>` flags also repeat? (three places pay)

`--ext md --ext mdx` parses fine and reads well, and the objection is not in
the parser. The reference page has to document both spellings. The integration
tests have to pin both, for the reason 0016 § stress test 4 gives: a behavior
that can be reached two ways needs a test on each way, or the untested one
drifts. And the config key that mirrors the flag (`exclude:` is a YAML list) has
to say which spelling it corresponds to. 0005 declined the same thing for
`--fields`, and the reasoning has not changed.
...
A one-flag change has a three-rung ladder: the command without the flag, with
it, and the usage error if the flag takes a value. If those three lines are hard
to write, the flag is not designed yet, which is what the ladder is for.
