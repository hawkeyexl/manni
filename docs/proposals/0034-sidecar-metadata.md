# 0034: sidecar metadata, a private manifest joined to public documents

- **Status:** Implemented
- **Serves:** Maya · M1, M2 · Devin · D1, D4 · Sara · S1
- **Depends on:** [0004](0004-config-upward-discovery.md) (config-relative paths;
  the config directory is the one base a manifest's keys resolve from),
  [0014](0014-empty-input-is-not-success.md) (a named input that is not there is
  an error, and that includes a manifest entry naming a document the run did not
  load), [0018](0018-write-support-shipped-for-all-three.md) (a write updates the
  location the value was read from, which is why the first increment refuses
  writes to sidecar keys rather than writing them into the document),
  [0020](0020-element-metadata.md) (both channels are validated, no precedence
  tiebreak; a document carrying a sidecar-owned key is a finding), [0026](0026-corpus-checks-are-findings.md)
  (a row naming a path outside the run is exit 2, not a synthetic finding, and a
  corpus invariant is checked only when the run is the config corpus)
- **Relates to:** [0006](0006-gitignore-aware-discovery.md) (the multi-root
  filtering defect this proposal fixes first), [0015](0015-schema-trust-boundary.md)
  (a sidecar never feeds schema resolution), [0031](0031-input-formats-notebooks-and-markdoc.md)
  (a manifest is a join table, not the standalone data file 0031 rejected),
  [0001](0001-validation-baseline.md) (the new finding rides the baseline
  unchanged)
...
things that are not: the internal design note a guide was written from, the
ticket that tracks it, the team that owns it. Today that metadata has two
homes, and both are wrong. In the public frontmatter it leaks. In a private
...
plain config and never mentions a sidecar, so no public run ever warns about a
private file it cannot see. Running from the private checkout is what "evaluate
against the public docs when both are available" means.
...
1. **Keys are owned.** Each sidecar entry declares the top-level keys it
   ...
   an operational error, and so is `$schema`, which no sidecar may own (0015: a
...
3. **Manifest keys are exact paths, relative to the config's directory.** No
   ...
   that a silently merged result changes what the contract means, and this
   repository has no specificity metric to borrow. "Every guide gets X" is a
   follow-up entry form, recorded below, not a manifest feature.
4. **An entry naming a document the run did not load is exit 2**, following
...
5. **A sidecar-sourced violation names the sidecar.** The finding's subject
   ...
   since that is where the fix goes, and each SARIF finding resolves its own
...
Manifest: a YAML mapping from document path to a mapping of owned key to value.
A top level that is not a mapping, an entry that is not a mapping, or an entry
key outside `keys:` is exit 2 naming the manifest and the entry.
...
and the `sidecar` first segment is reserved in the registry beside `check`.
...
### The prerequisite: gitignore across roots

The recommended layout puts documents in a different git root from the config.
Today one such candidate makes `git check-ignore` exit 128, `gitignore.ts`
reads that as "git unavailable" for the whole batch, and filtering silently
...
- **Mirror tree, one sidecar per document.** Rejected for the first increment.
  ...
  is the silent green 0014 exists to end, and per-document files leave nowhere
...
- **A `$meta:` pointer in the public document.** Rejected. In public CI it is a
  named file that is not there, and a contributor choosing what is merged into
...
1. **Four adversarial reviews, four lenses.** Precedent, pipeline fit, the CI
   ...
   you read it from" without a declared owner, and a doc-side private key with
