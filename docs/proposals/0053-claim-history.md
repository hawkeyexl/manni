...
1. `HEAD` first. When `HEAD` holds the pin and the working tree does not, the
   change is uncommitted. The baseline is `HEAD`, zero commits since.
...
3. At some commit, the pin string may appear in neither the page nor the
   manifest that owns the entry. Then the walk has passed the entry's birth.
   It stops, and the claim has no baseline.
4. When git has no parent to walk to, as in a shallow clone, the history is
   unavailable.
5. The walk stops after 256 commits that touched the page, and the history is
   unavailable.
...
- **A marker-anchored entry.** Find the marker naming the entry's id in that
  commit's body. Try every span that starts on the marker's line, or on a line
  before the first prose line below it. Each span ends at or before that
  paragraph's end, the next blank line or fence.
...
One run gives new lines. Several runs are `claim-moved-ambiguous`, as they are
for verbatim text. None is `claim-changed`.
...
- **`claim-changed`.** Skipped and reported, as today. With `--accept`,
  re-pinned as today. The report line gains the baseline.
...
`severity:` takes fifteen rule names, where it took fourteen.
...
| `update` | `--accept` | Unchanged in what it re-pins. Its report line for a claim names the baseline. |
...
| Words edited, change uncommitted | `fetch-timeout: the claim at line 9 has changed since 3f9c2a1, 0 commits.` | warning | 0 |
...
| A changed claim accepted | `docs/limits.md: fetch-timeout claim at line 9 re-pinned (changed since 3f9c2a1, 2 commits; now "The fetch timeout is 30 seconds.")` | 0 |
...
**update json.** A rewrite's `reason` and `status` each gain `"reanchored"`.
For a reanchored claim, `from` and `to` are the pins, `at` is its first file
line, and `commitSha` is the baseline. `fileLines` is where a claim-lines
entry now sits.
...
**5. Accept one, after reading it.**

```console
$ manni cite update --accept --only anchor-links docs/src/content/docs/a11y/index.mdx
docs/src/content/docs/a11y/index.mdx: anchor-links claim at line 53 re-pinned (changed since 9d4cdd8, 1 commit; now "A link's fragment is dropped, and the link is checked as its page.")
...
{ "id": "retries", "reason": "reanchored", "commitSha": "3f9c2a1e7b0d4c5a6f8e9d0b1a2c3d4e5f607182" }
{ "id": "page-size", "reason": "reanchored", "commitSha": "5d1e0b7c2a9f41e08b6d3a5c7e9f1b2d4a6c8e0f" }
# exit 0
```
...
| `UpdateRewrite` | interface | `reason` and `status` gain `"reanchored"`. Gains `fileLines`. `commitSha` also carries a claim's baseline. |
| `GitClient` | interface | Gains an optional `pageCommits(path)`. A client without it reads no claim history. |
| `claimHistory` | function | New. Refines a `changed` claim end against the page's history. |
...
**Changed as a result:** nothing, and it is an open question. Refusing would
break the documented `update --accept --dry-run -f json docs/` form. The
...
A config that sets the fourteen existing rules still loads.

Three things change behaviour, and each is a `feat(cite):`.
...
1. **Should a widened anchor count as reanchored?** Condition 1 lets the
   anchor now cover words the old pin did not. Those words were on the page,
   but no one checked them against the source. The alternative is equality
   only, which leaves every hoisted marker a warning.
2. **Is 256 commits the right cap?** It bounds a page with a long history.
   A docs site with daily bot commits to one page may reach it.
3. **Should `update --accept` without `--only` refuse when claims changed?**
   Stress test 10 records why this proposal does not.
4. **Should the walk follow a renamed page?** It is the page being checked,
   so its identity is not in doubt. Following renames costs a
   `git log --follow` per page and is heuristic.
5. **Is `reanchored` the right word?** It names the common cause. A rewrapped
   paragraph with no marker in sight is also reanchored under this rule.
