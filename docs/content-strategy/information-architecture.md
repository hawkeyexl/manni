...
The tree above is the `meta` group. The nav gains a `cite` group beside it, a sibling with the same shape: Overview, Get started, Set up, Run it in CI, Fix a failing check, Reference, each autogenerated from the matching directory under `cite/`. There is no schemas track, because the vocabulary a citation is written in is meta's (`manni:citations`, proposal 0035) and lives on meta's proposals hub until it registers.
...
The second tool's content set, under `docs/src/content/docs/cite/`. Same persona tracks, three new CUJs (M5, D5, T2), and the same rule that journey pages link into the reference shelf rather than restating flag tables. Every page names `manni cite` freely; these are the domain's own pages.

| Page | CUJ | ★ | Notes |
|---|---|---|---|
| Overview (`cite/index.mdx`) | All | ★ | `template: splash`, mirroring `meta/index.mdx`. What a citation is in two sentences, the three commands, a 30-second proof from real `add` and `check` output, links to the journey pages and the reference pages. |
...
| Public docs, private code (`cite/set-up/public-docs-private-code.mdx`) | M5, D5 | ★ | The two-repo layout from 0035: docs config with `obfuscate: true`, public CI `--no-sources`, private CI from the docs checkout with `--root ../code` and `MANNI_CITE_SALT` from a secret. What a token is and is not, that ids and claims leak by name, that the salt stays out of public files, that no reporter prints a resolved path unless `--reveal`. |
...
| Citations reference (`cite/reference/citations.mdx`) | `docs/proposals/0035/schemas/citations/1.0.0-proposal.1.json` (the entry shape, `src` grammar, hashing rule), `src/cite/core/statements.ts` (the inline forms and anchors) |
