...
| Artifact coverage (`tracevals/declare/coverage.mdx`) | M17 | ★ | The six states a coverage row can be in, the resolution order for skills, agents, slash commands and project rules, and how to diagnose an unresolved reference from the `tried` list. Coverage is the honest answer to "what did you not check?". |
| Run it in CI (`tracevals/ci/index.mdx`) | D10 | ★ | Leads with the offline answer rather than arriving at it. `--deterministic-only` as a complete mode, `--local` for runs that must not leave the runner, `CLAUDE_CONFIG_DIR` pinned on a shared runner, and recipes with every third-party action pinned to a full SHA. |
...
| Read a failing eval (`tracevals/triage/index.mdx`) | T5 | ★ | Complete in one page, with no subject dependencies. The five outcomes kept distinct, the several unrelated causes of `SKIP` kept apart, a coverage row named as not an outcome, and "the eval is wrong" named beside "fix the behavior" and "escalate". |
