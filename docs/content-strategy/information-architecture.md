...
| Citations reference (`cite/reference/citations.mdx`) | M5, T2 | ★ | The entry shape (`src`, `integrity`, `commit`, `id`, `claim`, `quote`), the `src` grammar and encrypted sources, the hashing rule and the keyed pin, the inline statement forms per format, and the anchor rule. Source of truth: the draft vocabulary and `src/cite/core/statements.ts`. |

### `key/` (the family encryption key)

The third domain's content set, under `docs/src/content/docs/key/`. `key` is not a document tool. It manages a family resource, the one encryption key every tool encrypts values with (proposal 0045), so it has no journey track of its own. Its two pages serve the two journeys that need the key. Sara's S1 needs a field required on every page whose value must never appear in plain text. Devin's D5 needs private source paths in public docs. The nav gains a `key` group after `cite`, with an Overview link and a Reference group autogenerated from `key/reference/`. The keyword's contract stays on meta's schema resolution page, and the `encryptionKey:` contract on meta's configuration page; these pages link to both.

| Page | CUJ | ★ | Notes |
|---|---|---|---|
| Overview (`key/index.mdx`) | S1, D5 | ★ | `template: splash`, mirroring `a11y/index.mdx`. What the key is for, told through S1 (`x-manni-encrypt` on `owner`) and D5 (encrypted `src` paths). The two verbs, and a 30-second proof: `key set`, a page holding `owner: ~AQx7…`, `meta validate` passing, `key rotate`. Where the key lives (top-level `encryptionKey:`, `MANNI_ENCRYPTION_KEY` wins), the committed-key warning and the environment-variable layout for a shared repository, and the write prompt. |
| CLI reference (`key/reference/cli.mdx`) | S1, D5 | ★ | `set`/`rotate`; every argument and flag, and the exit codes. How rotation finds values: by ciphertext, every value or none, resumable, the narrowed-run and environment rules, and the citation baseline. Then every refusal, and the ladder from minimal to maximal. Guarded by `npm run docs:check-cli`. Source of truth: `src/key/cli.ts`. |
...
| Citations reference (`cite/reference/citations.mdx`) | `docs/proposals/0044/schemas/citations/1.0.0-proposal.2.json` (the entry shape, `src` grammar, hashing rule), `src/cite/core/statements.ts` (the inline forms and anchors) |
