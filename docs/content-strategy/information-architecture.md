...
| Create your `manni.config.yaml` | M1 | ★ | The file's two halves. The top-level `collections:` list (name, paths, exclude, url, externalMetadata), then the `meta:` keys (schemas, overrides by `collection:` or `files:`), plus discovery, with types and defaults. Carries why `collections:` is nobody's tool key, the moved-key refusals for `meta.paths` / `meta.exclude`, `--collection`, and the 1.0 change that a collection's `exclude` decides membership rather than filtering a typed path. Source of truth: `src/shared/collections.ts`, `src/shared/config-file.ts`, `src/meta/core/config.ts`. |
