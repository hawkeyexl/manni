...
**Outcome.** A sentence on one of Maya's pages rests on one line of code, and she finds out when that line changes, before a reader does. The finding names the sentence, not the page.

**Steps.** The package is the one she already has. She runs `manni cite add <page> <path:line> --claim "…"` and the tool writes a `citations` entry (source range, hash, commit) plus a reference statement above the sentence. `manni cite check` on every push classifies each entry as `current`, `moved` or `changed`, from git alone, with no model and no network. When the source gains lines above the cited one, `manni cite update` rewrites the range. When the cited line itself changes, she reads the diff with `--show-diff`, decides whether the prose or the pin is wrong, fixes the sentence, and re-mints with `manni cite update --accept`. Once the frontmatter channel is familiar, she cites inline with `--inline`, in the format's comment syntax, and pins a fenced code block with `--quote`.
...
**Outcome.** Every pull request reports which cited sentences its source changes made stale, as annotations on the sentence line, and a citation that went stale in a commit that touched no page does not block an unrelated PR.

**Steps.** Devin adds `manni cite check` beside the metadata gate. He picks the output his tooling reads: `-f github` for inline PR annotations, `-f sarif` for code scanning, `-f junit` for the test tab. He learns the contract: exit `0` clean, `1` an error-severity finding, `2` operational; `moved` and `claim-ambiguous` are warnings and never fail the job. He sets `fetch-depth: 0` on the checkout, because a shallow clone cannot show the file at the commit a pin was minted at, and without it `never-true` and the diff degrade to a notice. For the ramp-in he runs the PR job with `--baseline`, report-only, and a scheduled sweep without it that fails. Where the docs are public and the code is not, the public job runs `--no-sources` and the private job runs from the docs checkout with `--root ../code` and the salt from a secret.
