 ...
 */
export function ignoredFields(raw: Partial<Record<TermField, unknown>>): IgnoredField[] {
  ...
  for (const field of TERM_FIELDS) {
    ...
    const given = Array.isArray(value) ? "list" : typeof value === "object" ? "mapping" : "other";
    ignored.push({ field, given });
  }
  return ignored;
}

/** A one-value field left out of a record, and what it was given instead of text. */
export interface IgnoredField {
  field: TermField;
  given: "list" | "mapping" | "other";
}

/**
 * The notice for a one-value field `ignoredFields` found. It names what the
 * field holds and, for a list or a mapping, what it was given.
 */
export function ignoredNotice(input: TermInput, line: number, label: string, ignored: IgnoredField): string {
  const { field, given } = ignored;
  const article = /^[aeiou]/.test(field) ? "an" : "a";
  const holds =
    given === "list" ? "holds one value, not a list" : given === "mapping" ? "holds text, not a mapping" : "holds text";
