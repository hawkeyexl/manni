...
export async function runVendorSchema(
  ...
  if (!/^https?:\/\//i.test(url)) {
    throw new DocmetaError(
      `\`manni meta schemas vendor\` takes an http(s) URL to download; "${url}" is ${/^[a-z0-9][a-z0-9._-]*:/i.test(url) && !url.includes("/") ? "a built-in id, which is already bundled" : "a local reference, which is already in your repository"}.`,
    );
  }
...
export async function runInferSchema(
  ...
  // Sidecar manifests (0034): an inferred schema describes the merged corpus.
  ...
  const draft: Record<string, unknown> = {
    ...
    $comment:
      "Generated by `manni meta schemas infer`. Nothing is required, deliberately: what your docset happens to contain is not the same as what your standard should demand. Promote keys into a `required` list yourself, one at a time, behind a baseline.",
    type: "object",
    properties,
  };
