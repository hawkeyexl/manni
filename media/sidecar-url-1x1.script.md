...
**Objective:** Show that `sidecars[].file` now takes a URL, so one repository's
pages can be validated against a manifest served from another, with no checkout
and no shell step; that a private manifest needs only `tokenEnv:`; and that a
manifest which cannot be reached is exit 2, never a silent pass.
...
- **A Remotion replay, not a VHS capture.** VHS cannot record on this machine:
  its bundled `ttyd` hangs on connect (re-checked this run with a one-line tape;
  it produced no frame in 60 s). So the CLI was run under a small preload that
  makes stdout/stderr report as a TTY (`media/capture/tty.cjs`), the bytes were
  saved verbatim (`media/capture-url/*.ans`), and a Remotion composition
  (`media/remotion/src/url/beats.ts`, `src/Demo.tsx`) replays them: typing at
  45 ms/char, Enter, then the output after the command's **measured** latency.
  Nothing in the output is edited.
- **The server is local.** There is no public repository to fetch from, so the
  ...
  on loopback, which the feature permits for `http://` (any other host must be
  `https://`). The URL on screen is the real one, `http://127.0.0.1:8765/…`.
...
- **`media/scratch-url/` is its own git repository.** `media/` is gitignored
  ...
  the scratch directory makes it the "public repo" of the story and takes the
  outer ignore file out of the picture, with no extra flag on any typed command.
...
- **Long lines wrap at spaces, with a hanging indent.** A real terminal would
  ...
  gives the continuation row the line's own indent, so no token is split
...
line wraps to exactly two rows with no orphaned last word (24 px puts the
`docs/ops.md` finding on three rows; 25 px orphans `set.`). Line height 32 px.
...
"never cached" (0038 rule 1; `src/meta/core/sidecar-fetch.ts` has no cache
path) and "never the token" (0038 rule 4; the beat 3 message names the
variable `PRIVATE_DOCS_TOKEN`, and the token value is only ever placed in the
...
Also verified, not shown: with `PRIVATE_DOCS_TOKEN=abc` set, the private
config produces the beat 2 output and exit 1 (the local server ignores the
header, so the run proves the fetch, not the authentication).
...
- Output appears after the command's real measured latency, three runs each:
  validate 563-591 ms (includes the fetch), token-unset 532-556 ms, offline
  521-547 ms. See `latency` in `media/remotion/src/url/beats.ts`.
