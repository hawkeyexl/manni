...
frontmatter field instead of its path, so a rename never orphans an entry;
that two pages sharing one id is a finding on both; and that `query` refuses
to change the id of a page the manifest matched, with exit 2.
...
- **A Remotion replay, not a VHS capture.** VHS cannot record on this machine:
  its bundled `ttyd` hangs on connect (re-checked this run with a one-line
  tape; VHS echoed the tape header and produced no frame in 60 s). So the CLI
  was run under a small preload that makes stdout/stderr report as a TTY
  (`media/capture/tty.cjs`), the bytes were saved verbatim
  (`media/capture-join/*.ans`), and a Remotion composition
  (`media/remotion/src/join/beats.ts`, `src/Demo.tsx`, composition
  `SidecarJoinDemo`) replays them: typing at 40 ms/char, Enter, then the
  output after the command's **measured** latency. Nothing in the output is
  edited.
- **`media/scratch-join/` is `test/fixtures/sidecars-join/`, staged.** It is
  ...
  1. `docs/new.md` and `docs/no-id.md` were removed. Both fail on missing
     required keys, which is the schema's `required` rule doing its job, but
     it is not what this video is about and there is no caption room to
     explain it. `docs/billing.md` stays: removing it would orphan its
...
- **The rename is on camera.** Beat 2 types
  ...
  have raised the orphan error for a path-keyed manifest (0039 rule 3 runs
  the orphan check only on a corpus run), so its exit 1 with a green check on
  the new path is the demonstration. A scoped run would not have proved it.
...
- **Long lines wrap at spaces, with a hanging indent.** A real terminal would
  ...
  the continuation row the line's own indent, so no token is split
...
`[sidecar:duplicate]` lines wrap to exactly two rows and every continuation
row carries two or more words (28 px puts each on three rows with the tag
alone on the last; 27 px orphans four last words). Line height 36 px. The
tallest beat is 20 rows = 720 px of the 838 px available. 23 px / 75 columns,
which the URL video derived, also fits; 26 px was chosen because these lines
are shorter and the type can be larger.
...
Highlighted rows: beat 1 `join: id` and the three manifest keys; beat 2
`✓ docs/guides/authentication.md`; beat 3 the four `dup-a` / `dup-b` rows;
...
- Output appears after the command's real measured latency, three runs each:
  validate 593-614 ms, query 585-605 ms; the replay uses 610 ms for both.
