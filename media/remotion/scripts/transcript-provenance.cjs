...
const b = require("./out-provenance/provenance/beats.js");
...
let out = `Provenance pins, demo video transcript (silent video; text describes what is on screen)\n\nFrame: 1080x1080, 30 fps, ${total} s. Every terminal line is real output of git and of\n\`node dist/cli.js meta ...\` (typed as manni) run inside media/scratch-provenance/,\na git repository built from test/fixtures/derive/provenance/.\n\n`;
let t = 0;
b.beats.forEach((beat, i) => {
  ...
  for (const c of beat.commands) {
    ...
    if (c.output !== "") out += strip(c.output).replace(/\n$/, "").split("\n").map((l) => {
      ...
      if (/files? checked, 0 failed|0 failed, 0 errors/.test(l)) return `${l}   (green)`;
      if (/files? checked/.test(l)) return `${l}   (red)`;
      if (/^\+[^+]/.test(l) || l === "+") return `${l}   (green, added)`;
...
fs.writeFileSync("../provenance-pins-1x1.transcript.txt", out);
console.log(out);
