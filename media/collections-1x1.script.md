...
- **Feature:** the top-level `collections:` key (proposal 0041). A repository
  declares its document sets once, and every tool in the family reads the same
  declaration.
...
- **Accent:** `#58a6ff` blue, the palette value. Not red, green, yellow or
  cyan, all four of which manni's own output spends in this very frame:
  green `✓` and green summaries in `meta validate` and `a11y check`, cyan URLs
  in the a11y report.
...
`SELECT … FROM guides`) demonstrates narrowing, which is a smaller claim — one
tool, one config key, and no reason the key had to move to the top level. Here
...
`media/scratch-collections/`, with `media/capture/tty.cjs` making the CLI
...
- The site is clean, so the a11y beat ends at `0 violations`. That is a
  ...
  `dequeuniversity.com` help URL 79 characters long, which cannot be wrapped at
  a space and so would have forced the whole video down to 20 px type. The beat
...
over every real line at each candidate size (the algorithm is copied from
...
|---|---|---|---|
| 1 | One set, named twice | `cat manni.config.yaml` — the old shape | One document set, spelled twice: a glob under meta:, a URL under a11y:. Move the docs and one goes stale. |
...
| 3 | Declared once | `cat manni.config.yaml` — the new shape | One collection: the paths, and the url those pages are published at. No tool owns it. |
...
highlights the two file paths, beat 5 the three page URLs — the same collection,
seen by two tools as two different kinds of thing.
...
than a warm one. 2.00 s is the middle of the four measured runs. Disclosed here
rather than trimmed.
...
5. Loudness measured on the finished file. **N/A** — the video is silent. The
