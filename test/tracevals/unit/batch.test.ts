...
let tmpDir: string;
beforeAll(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), "manni-tracevals-batch-"));
});
afterAll(async () => {
...
describe("runBatch", () => {
  ...
  describe("--history", () => {
    it("appends one entry per trace", async () => {
      const historyDir = await mkdtemp(join(tmpdir(), "manni-tracevals-batch-history-"));
      try {
        await batch({ history: true, configDir: historyDir });
        const { loadHistory } = await import("../../../src/tracevals/history.js");
        const entries = await loadHistory(
          join(historyDir, ".manni", "tracevals", "history.jsonl"),
        );
