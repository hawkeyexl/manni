...
/**
 * A managed field, or a `derive.commands` key, that a manifest already owns.
 * The rule needs `meta.derive` and the top-level `collections:`, which the
 * section parser never sees, so the refusal comes from `loadConfig`, as the
 * undeclared-collection one does.
