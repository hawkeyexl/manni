/**
 * The forge source: `reviewed-by` and `last-reviewed` from the PR/MR that
 * merged a commit, reached through `gh` / `glab`.
 *
 * Nothing here touches a real forge. The clients spawn
 * `test/helpers/fake-forge-bin.mjs` through `bin: process.execPath`, so the
...
const GITLAB: ForgeIdentity = { kind: "gitlab", host: "gitlab.com", project: "group/sub/docs" };
...
describe("identityFromOrigin", () => {
  ...
  it("treats any other host as a GitHub Enterprise host", () => {
    expect(identityFromOrigin("ghe.example.com", "acme/docs")).toEqual({
      kind: "github",
      host: "ghe.example.com",
      project: "acme/docs",
    });
...
describe("GhClient", () => {
  ...
  it("answers null for an open PR", async () => {
    ...
    expect(await new GhClient(GITHUB, fake.spawn).mergedChangeFor(SHA)).toBeNull();
  ...
  it("treats a 404 from the forge as no change, not as a failure", async () => {
  ...
  it("reports an unauthenticated gh with the fix", async () => {
    ...
    const status = await new GhClient(GITHUB, fake.spawn).status();
  ...
  it("reports a missing binary with the fix", async () => {
    fake = fakeForge([AUTH_OK]);
    const client = new GhClient(GITHUB, {
      ...fake.spawn,
  ...
  it("reports a missing origin without blaming gh", async () => {
    fake = fakeForge([AUTH_OK]);
    const client = new GhClient(null, fake.spawn);
    expect(await client.detect()).toBeNull();
    expect(await client.status()).toEqual({
      available: false,
      reason: "no origin remote to derive the forge from",
    });
  ...
  it("throws on a malformed answer rather than reading it as no approvals", async () => {
    ...
    await expect(new GhClient(GITHUB, fake.spawn).mergedChangeFor(SHA)).rejects.toThrow(
      /did not return JSON/,
    );
  });
...
describe("GlabClient", () => {
  ...
  it("finds the merged MR and stamps its approvals with the merge time", async () => {
    fake = fakeForge(MR_SCENARIO);
    const client = new GlabClient(GITLAB, fake.spawn);
    expect(await client.status()).toEqual({ available: true });
  ...
  it("answers null when no MR merged the commit", async () => {
    ...
    expect(await new GlabClient(GITLAB, fake.spawn).mergedChangeFor(SHA)).toBeNull();
  });

  it("names glab in its status reasons", async () => {
    fake = fakeForge([{ includes: ["auth", "status"], exit: 1 }]);
    expect((await new GlabClient(GITLAB, fake.spawn).status()).reason).toBe(
      "glab is not authenticated for gitlab.com; run glab auth login",
    );
    const missing = new GlabClient(GITLAB, {
      ...fake.spawn,
...
describe("ForgeCache", () => {
  ...
  it("lives under .manni/meta/forge-cache", () => {
    expect(FORGE_CACHE_DIR).toBe(".manni/meta/forge-cache");
  });

  it("round-trips a merged change under a digest-named file", () => {
    dir = realpathSync(mkdtempSync(join(tmpdir(), "manni-forge-cache-")));
    const cache = new ForgeCache(join(dir, "cache"), true);
  ...
  it("does nothing when disabled", () => {
    dir = realpathSync(mkdtempSync(join(tmpdir(), "manni-forge-cache-")));
    const cache = new ForgeCache(join(dir, "cache"), false);
    cache.set("k", CHANGE);
...
describe("cachedClient", () => {
  ...
  it("serves a merged answer from disk without spawning again", async () => {
    dir = realpathSync(mkdtempSync(join(tmpdir(), "manni-forge-cache-")));
    fake = fakeForge(ghScenario());
    const cache = new ForgeCache(join(dir, FORGE_CACHE_DIR), true);
    const client = cachedClient(new GhClient(GITHUB, fake.spawn), cache, GITHUB);
    ...
    const again = cachedClient(
      new GhClient(GITHUB, fake.spawn),
      new ForgeCache(join(dir, FORGE_CACHE_DIR), true),
      GITHUB,
    );
    expect(await again.mergedChangeFor(SHA)).toEqual(first);
    expect(fake.calls().length).toBe(spawnsAfterFirst);

    const entry = readdirSync(join(dir, FORGE_CACHE_DIR));
    expect(entry).toHaveLength(1);
    const raw = JSON.parse(readFileSync(join(dir, FORGE_CACHE_DIR, entry[0] ?? ""), "utf8"));
  ...
  it("spawns again when the cache is off", async () => {
    dir = realpathSync(mkdtempSync(join(tmpdir(), "manni-forge-cache-")));
    fake = fakeForge(ghScenario());
    const cache = new ForgeCache(join(dir, FORGE_CACHE_DIR), false);
    const client = cachedClient(new GhClient(GITHUB, fake.spawn), cache, GITHUB);
  ...
  it("passes status and detect through, and bypasses the cache with no identity", async () => {
    ...
    fake = fakeForge([AUTH_OK]);
    const cache = new ForgeCache(join(dir, FORGE_CACHE_DIR), true);
    const client = cachedClient(new GhClient(null, fake.spawn), cache, null);
    expect(await client.detect()).toBeNull();
...
describe("createForgeClient", () => {
  ...
  it("reads the origin, picks the CLI, and caches under the root", async () => {
    ...
    expect(readdirSync(join(repo, FORGE_CACHE_DIR))).toHaveLength(1);
  });
...
/** A `ForgeClient` with canned answers, for the pure half of the source. */
function canned(
  identity: ForgeIdentity | null,
  changes: Record<string, MergedChange | null>,
  available = true,
): ForgeClient & { looked: string[] } {
  const looked: string[] = [];
  return {
    looked,
...
describe("deriveFromForge", () => {
  ...
  it("derives both fields from a GitHub PR, reviewers in approval order", async () => {
    ...
    const result = await deriveFromForge([{ label: "docs/a.md", sha: SHA }], client);
    ...
    expect(result.records.get("docs/a.md")).toEqual({
      ...
      "last-reviewed": { value: "2026-03-02", source: "forge", evidence: "github PR #18" },
    });
  });
  ...
  it("uses the UTC date of the latest approval", async () => {
    ...
    const result = await deriveFromForge([{ label: "a.md", sha: SHA }], canned(GITHUB, { [SHA]: change }));
  ...
  it("says where a GitLab date comes from", async () => {
    ...
    const result = await deriveFromForge([{ label: "a.md", sha: SHA }], canned(GITLAB, { [SHA]: change }));
    expect(result.records.get("a.md")).toEqual({
      "reviewed-by": { value: ["maya"], source: "forge", evidence: "gitlab MR !18" },
      "last-reviewed": {
        value: "2026-03-02",
        source: "forge",
  ...
  it("derives null for a working-tree document, and never asks the forge", async () => {
    const client = canned(GITHUB, { [SHA]: CHANGE });
    const result = await deriveFromForge([{ label: "new.md", sha: null }], client);
  ...
  it("asks the forge once per sha, however many documents share it", async () => {
    const client = canned(GITHUB, { [SHA]: CHANGE });
    await deriveFromForge(
  ...
  it("reports an unavailable forge without calling it", async () => {
    const client = canned(GITHUB, { [SHA]: CHANGE }, false);
    const result = await deriveFromForge([{ label: "a.md", sha: SHA }], client);
  ...
  it("runs end to end through the gh client", async () => {
    ...
    expect(result.records.get("a.md")).toEqual({
      "reviewed-by": { value: ["maya", "devin", "claude"], source: "forge", evidence: "github PR #18" },
      "last-reviewed": { value: "2026-03-02", source: "forge", evidence: "github PR #18" },
    });
  });
});
