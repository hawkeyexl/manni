...
describe("applyKgFields", () => {
  it("adds a graph map to existing frontmatter, preserving body byte-for-byte", () => {
    const content = `---\ntitle: T\ntags: [x] # keep me\n---${BODY}`;
  ...
  it("nests a dotted field when the file has no frontmatter at all", () => {
    // The no-frontmatter branch builds the whole `graph` map from scratch, and a
  ...
  it("leaves a non-map where a section path needs one, and reports it", () => {
    // A human wrote `graph.sections: none`. Writing through it would destroy their
  ...
  it("refuses a graph key that is not a map, by name", () => {
    const content = `---\ntitle: T\ngraph: not-a-map\n---${BODY}`;
    expect(() => applyKgFields(content, "a.md", { label: "X" })).toThrow(
      'a.md: frontmatter key "graph" is not a map',
    );
  });
  ...
  /**
   * `options.page` writes beside the `graph` map rather than inside it, so a fill
   * is still one parse and one serialization (proposal 0046's
   * `meta-provenance` is the one caller).
   */
  describe("options.page", () => {
    const RECORD = [{ "generated-by": "m1", fields: ["/graph/label"] }];

    it("writes a top-level key beside graph, keeping the body byte-for-byte", () => {
      const content = `---\ntitle: T\n---${BODY}`;
      ...
      expect(data["graph"]).toEqual({ label: "X" });
      expect(result.content.endsWith(BODY)).toBe(true);
    });
    ...
    it("writes even when no graph field does, and mints no empty graph map", () => {
      const content = `---\ntitle: T\n---${BODY}`;
      const result = applyKgFields(
        content,
        "a.md",
      ...
      expect(result.content).not.toContain("graph:");
    });
...
describe("existingKgFields", () => {
  it("lists fields present on the graph map", () => {
    const content = `---\ngraph:\n  label: X\n  concepts: [a]\n---\n`;
    expect(existingKgFields(content).sort()).toEqual(["concepts", "label"]);
  });

  it("returns [] without frontmatter or graph key", () => {
