...
import type { Quad, Term } from "../../../src/kg/core/derive.js";
import type { DeriveSource, RouteMapping } from "../../../src/kg/core/config.js";
import type { GitHistory } from "../../../src/kg/core/git.js";
...
describe("deriveGraph — document basics", () => {
  it("types every doc and records its path", () => {
    ...
    expect(has(g, DOC, `${NS.rdf}type`, iri(`${NS.kg}Document`))).toBe(true);
    expect(has(g, DOC, `${NS.kg}path`, lit("docs/a.md"))).toBe(true);
  });
...
describe("deriveGraph — provenance", () => {
  ...
  it("maps kg.derived-from to resolved docs, URLs, and broken links", () => {
    ...
    expect(has(g, DOC, `${NS.kg}brokenLink`, lit("missing.md"))).toBe(true);
  });
  ...
  it("keeps a '## Generation' heading distinct from the generation activity", () => {
    ...
    const activity = `${DOC}#prov.generation`;
    expect(has(g, section, `${NS.rdf}type`, iri(`${NS.kg}Section`))).toBe(
      true,
    );
    ...
    expect(has(g, activity, `${NS.rdf}type`, iri(`${NS.kg}Section`))).toBe(
      false,
    );
  });
  ...
  it("maps meta-provenance /kg/ pointers to per-model fill activities, not shared subjects", () => {
    ...
    expect(
      has(g, claudeActivity, `${NS.kg}filledFieldEntry`, iri(claudeEntry)),
    ).toBe(true);
    expect(has(g, claudeEntry, `${NS.kg}filledField`, lit("label"))).toBe(
      true,
    );
    ...
    expect(has(g, gptEntry, `${NS.kg}filledField`, lit("concepts"))).toBe(
      true,
    );
  ...
  it("reads a deep pointer's first segment as the filled field", () => {
    ...
    expect(g.filter((q) => q.p === `${NS.kg}filledField`)).toHaveLength(1);
  });
  ...
  it("unescapes a pointer segment per RFC 6901", () => {
    ...
    expect(
      has(g, `${base}.field.alt/labels`, `${NS.kg}filledField`, lit("alt/labels")),
    ).toBe(true);
    expect(
      has(g, `${base}.field.a~b`, `${NS.kg}filledField`, lit("a~b")),
    ).toBe(true);
  });
  ...
  it("emits per-field confidence as a decimal on the fill-field entry (ADR 01015)", () => {
    ...
    expect(
      has(g, entry, `${NS.kg}confidence`, lit("0.9", `${NS.xsd}decimal`)),
    ).toBe(true);
  });
  ...
  it("emits the graph-level build activity with tool agent and prov:used edges", () => {
    ...
    const tool = `${BASE}agent/software/manni`;
    ...
    expect(has(g, tool, `${NS.kg}version`, lit("1.2.3"))).toBe(true);
  });
  ...
  it("maps declared kg.revision-of like derived-from: resolved, URL, or broken", () => {
    ...
    expect(has(g, DOC, `${NS.kg}brokenLink`, lit("gone.md"))).toBe(true);
  });
...
describe("deriveGraph — qualified provenance", () => {
  ...
  it("qualifies generation and build associations", () => {
    ...
    const buildAssoc = `${BASE}activity/build.assoc.manni`;
    expect(
      has(
        g,
        `${BASE}activity/build`,
...
describe("deriveGraph — sections", () => {
  it("nests sections under doc and parent sections with level and order", () => {
    ...
    const secA = `${DOC}#a`;
    const secB = `${DOC}#b`;
    expect(has(g, secA, `${NS.rdf}type`, iri(`${NS.kg}Section`))).toBe(true);
    expect(has(g, DOC, `${NS.dcterms}hasPart`, iri(secA))).toBe(true);
    expect(has(g, secA, `${NS.dcterms}hasPart`, iri(secB))).toBe(true);
    expect(has(g, secB, `${NS.kg}level`, lit("2", `${NS.xsd}integer`))).toBe(
      true,
    );
    expect(has(g, secB, `${NS.kg}order`, lit("1", `${NS.xsd}integer`))).toBe(
      true,
    );
  });
});
...
describe("deriveGraph — negative scope (ADR 01014)", () => {
  ...
  it("emits no negative-scope triples when the fields are absent", () => {
    ...
    expect(g.some((q) => q.p === `${NS.kg}notSoftwareSubject`)).toBe(false);
  });
});
...
describe("deriveGraph — images, code, derive toggles", () => {
  it("maps images and code languages", () => {
    ...
    expect(has(g, DOC, `${NS.kg}codeLanguage`, lit("python"))).toBe(true);
  });
...
describe("deriveGraph — localization (ADR 01037)", () => {
  ...
  it("records an unresolvable translation-of as a broken link", () => {
    ...
    expect(has(g, DE_DOC, `${NS.kg}brokenLink`, lit("../missing.md"))).toBe(
      true,
    );
    expect(
