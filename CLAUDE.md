...
A plan for a code change spells out the **user-facing interface**. That means
all of the following, in full:

- every config key, as the YAML a user writes, with its name, type, default,
  whether it is required, and what it does;
- every CLI argument and option;
- every message, with its exit code;
- every output shape the change adds or alters.

Show the config as a before/after document rather than as a type. The
programmatic API, meaning what `src/index.ts` exports, is listed when it
changes. It goes in a rename or addition table, after the user-facing surface,
and never instead of it. Internal types and function signatures are
implementation and stay out of the plan. "Add a flag for X" is a sentence, not
a plan. The review happens on the plan, so anything the plan leaves to be
invented during implementation is something the reviewer never saw. The first
time anyone looks at it is in the diff.

The review is about what a user will type and read, and that is why the split
runs where it does. A plan that opens with twelve interface declarations buries
the two config keys they will actually write, and the interfaces get rewritten
during implementation anyway. What cannot be rewritten cheaply is a config key,
a flag name, or an error message, because those ship.
